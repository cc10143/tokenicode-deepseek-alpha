...
11. **Model display routing** — `resolveModelForProvider` maps GUI model IDs to display names (provider mode: tier→provider mapping; inherit mode: `modelMappings[tier]` from settings.json). `resolveModelForSend` in inherit mode returns the selected tier's `_MODEL` value from settings.json (e.g. `claude-sonnet-4-6[1M]`, keeps the `[1M]` marker) so the GUI selection actually routes through CC Switch's substring tier matching; it returns `undefined` only when the tier has no `_MODEL` mapping, letting the CLI fall back to its own default (issue #7, fixed 2026-08-08). `resolveModelDisplay(rawModel)` is the single entry point for human-readable model names everywhere — maps CLI-reported names like `claude-sonnet-4-6[1M]` through settings.json if in inherit mode. Tier resolution uses substring matching (`tierFromModel`) rather than exact ID keys, because CLI-reported names carry version/date/context variants (`claude-haiku-4-5` without date suffix, `claude-opus-4-8[1M]`) that miss the fixed `TIER_MAP` keys (fixed 2026-08-08). `modelMappings` (transient, loaded once from `get_cli_model_mappings`) holds all tier→`{display, pass}` entries; `display` = `_MODEL_NAME` stripped of `[1M]` (dropdown), `pass` = `_MODEL` with `[1M]` preserved (the `--model` value). `inheritedModel` (transient, from `get_cli_model_config`) is legacy — loaded but no longer consumed.
