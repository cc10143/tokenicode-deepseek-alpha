...
fn system_proxy_url() -> Option<String> {
    ...
    if is_enabled("HTTPSEnable") {
        if let (Some(host), Some(port)) = (get_val("HTTPSProxy"), get_val("HTTPSPort")) {
            ...
            log::info!("system HTTPS proxy detected");
            return Some(url);
        }
    }
    if is_enabled("SOCKSEnable") {
        if let (Some(host), Some(port)) = (get_val("SOCKSProxy"), get_val("SOCKSPort")) {
            ...
            log::info!("system SOCKS proxy detected");
            return Some(url);
        }
    }
    if is_enabled("HTTPEnable") {
        if let (Some(host), Some(port)) = (get_val("HTTPProxy"), get_val("HTTPPort")) {
            let url = format!("http://{}:{}", host, port);
            log::info!("system HTTP proxy detected");
            return Some(url);
        }
    }
    None
...
fn probe_local_proxy() -> Option<String> {
    ...
    for &(port, scheme) in ports {
        ...
        .is_ok()
        {
            ...
            log::info!("auto-detected local proxy");
            return Some(url);
        }
    }
    None
    ...
    if let Some(proxy_url) = resolve_proxy_url() {
        if is_proxy_reachable(&proxy_url).await {
            if let Ok(proxy) = reqwest::Proxy::all(&proxy_url) {
                log::info!("Smart proxy: using configured proxy");
                builder = builder.proxy(proxy);
            }
        } else {
            log::info!("Smart proxy: configured proxy unreachable, connecting directly");
        }
    }
        ...
        if !purl.is_empty() {
            if let Ok(proxy) = reqwest::Proxy::all(purl) {
                if is_proxy_reachable(purl).await {
                    log::info!("test_provider_connection: using configured provider proxy");
                    reqwest::Client::builder()
                    ...
                    log::info!("test_provider_connection: configured provider proxy unreachable, direct");
